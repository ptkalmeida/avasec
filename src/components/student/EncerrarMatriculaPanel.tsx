...
export const EncerrarMatriculaPanel: React.FC<EncerrarMatriculaPanelProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 text-left shadow-3xs">
      <span className="mb-4 flex items-center gap-1.5 text-sobretitulo uppercase text-escult-ink-2">
        <BookOpen className="h-3.5 w-3.5" />
        <span>Minhas matrículas</span>
      </span>

      {aviso !== null && (
        <p className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-rotulo font-bold leading-relaxed text-emerald-800">
          {aviso}
        </p>
      )}
      ...
      {matriculas.length === 0 ? (
        ...
        <>
          <p className="mb-4 text-rotulo leading-relaxed text-escult-ink-2">
              ...
              <li key={curso.id} className="rounded-xl border border-slate-150 bg-slate-50/60 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <span className="block text-apoio text-escult-ink-2">{curso.category}</span>
                  </div>
                  ...
                  {confirmando !== curso.id && (
                    <button
                      ...
                      className="inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-lg bg-rose-600 px-3 py-2 text-sobretitulo uppercase text-white shadow-xs transition-colors hover:bg-rose-500"
                    >
                ...
                {confirmando === curso.id && (
                  <div className="mt-3 space-y-2 border-t border-slate-200 pt-3">
                    <p className="flex items-start gap-1.5 text-rotulo font-bold leading-relaxed text-slate-700">
                    ...
                    {erro !== null && (
                      <p className="rounded-lg border border-rose-200 bg-rose-50 p-2 text-apoio font-bold text-rose-700">
                        {erro}
                      </p>
                    )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() => sair(curso)}
                        className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg bg-rose-600 px-3 py-1.5 text-sobretitulo uppercase text-white hover:bg-rose-500 disabled:opacity-60"
                      >
                        <LogOut className="h-3.5 w-3.5" />
