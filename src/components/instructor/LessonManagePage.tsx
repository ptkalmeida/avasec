...
const ZoneHeader: React.FC<{
  ...
  <div className="flex items-start justify-between gap-3 border-b border-slate-150 px-4 py-2.5">
    <div className="min-w-0">
      <span className="flex items-center gap-1.5 text-sobretitulo uppercase text-slate-700">
        {icon}
        {title}
      </span>
      {hint && <p className="mt-0.5 text-apoio text-escult-ink-2">{hint}</p>}
    </div>
    <button
      type="button"
      onClick={onToggle}
    ...
    <div className="space-y-5 text-left animate-in fade-in duration-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-1.5 text-xs font-bold text-escult-ink-2 hover:text-slate-900 transition-colors cursor-pointer"
        >
        ...
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-sobretitulo uppercase text-escult-ink-2 border border-slate-200">
            {courseCategory}
          </span>
          <button
            type="button"
        ...
        {zone === 'header' ? (
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-sobretitulo text-escult-ink-2 uppercase mb-1">Carga</label>
                <input
                  type="text"
                  value={duration}
          ...
          <div className="p-4">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-rotulo font-bold text-teal-700">
              ...
              <span className="inline-flex items-center gap-1 text-escult-ink-2">
          ...
          <div className="p-4">
            {temVideo ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-6 text-center">
                ...
                <p className="mt-0.5 text-apoio text-escult-ink-2">
      ...
      <div className={caixa}>
        ...
        {zone === 'content' ? (
          ...
          <div className="p-4">
            <span className="mb-2.5 flex items-center gap-1 text-sobretitulo uppercase text-escult-ink-2">
              <Eye className="h-2.5 w-2.5" /> Como o aluno vê
            </span>
            {parsed.blocks.length === 0 ? (
              <p className="text-xs italic text-escult-ink-2">
            ...
            {docs.length === 0 ? (
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {docs.map((doc) => (
                  ...
                  >
                    <span className="rounded bg-white px-1.5 py-0.5 text-sobretitulo uppercase text-escult-ink-2 border border-slate-200">
                      {doc.type}
                    </span>
                    <span className="min-w-0 flex-1 truncate text-xs font-bold text-slate-800">{doc.title}</span>
                    {doc.size && <span className="shrink-0 text-apoio text-escult-ink-2">{doc.size}</span>}
                  </li>
                ))}
              </ul>
            )}
