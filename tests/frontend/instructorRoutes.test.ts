...
describe('curso antes da seção', () => {
  ...
  it('seção sem curso não existe: cai na escolha', () => {
    ...
    expect(parseInstrutor('/inst/grade-curricular').cursoRef).toBeNull();
    expect(parseInstrutor('/inst/curso').cursoRef).toBeNull();
    expect(parseInstrutor('/inst/curso/').cursoRef).toBeNull();
  });

  it('sem curso, o caminho é a raiz — mesmo pedindo uma seção', () => {
    expect(caminhoInstrutor({})).toBe(RAIZ_INSTRUTOR);
    expect(caminhoInstrutor({ cursoRef: null, secao: 'alunos' })).toBe(RAIZ_INSTRUTOR);
    expect(caminhoInstrutor({ cursoRef: '', secao: 'grade-curricular' })).toBe(RAIZ_INSTRUTOR);
  });
});
...
describe('endereço das seções', () => {
  it('o curso sozinho já é a Gestão do Curso', () => {
    ...
    expect(caminhoInstrutor({ cursoRef: 'course-1' })).toBe('/inst/curso/course-1');
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'gestao' })).toBe('/inst/curso/course-1');
    expect(parseInstrutor('/inst/curso/course-1').secao).toBe('gestao');
  });

  it('monta e lê cada seção', () => {
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'grade-curricular' }))
      .toBe('/inst/curso/course-1/grade-curricular');
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'alunos' }))
  ...
  it('ida e volta em todas as seções', () => {
    for (const secao of ['gestao', 'grade-curricular', 'avaliacoes', 'alunos', 'mensagens'] as const) {
      const caminho = caminhoInstrutor({ cursoRef: 'course-9', secao });
      expect(parseInstrutor(caminho)).toMatchObject({ cursoRef: 'course-9', secao });
    }
  });
  ...
  it('seção desconhecida mantém o curso e cai na entrada dele', () => {
    ...
    expect(d.cursoRef).toBe('course-1');
    expect(d.secao).toBe('gestao');
  });
  ...
  it('id com caractere especial sobrevive à ida e volta, barra inclusive', () => {
    ...
    const caminho = caminhoInstrutor({ cursoRef: id });
    expect(caminho).toContain('%2F');
    expect(parseInstrutor(caminho).cursoRef).toBe(id);
...
describe('sub-abas de Avaliações', () => {
  it('provas é o padrão e fica FORA do endereço', () => {
    ...
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'avaliacoes' }))
      .toBe('/inst/curso/course-1/avaliacoes');
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'avaliacoes', subAba: 'provas' }))
      .toBe('/inst/curso/course-1/avaliacoes');
  ...
  it('exercícios têm endereço próprio dentro de avaliações', () => {
    ...
    expect(caminhoInstrutor({ cursoRef: 'course-1', secao: 'avaliacoes', subAba: 'exercicios' }))
  ...
  it('sub-aba só vale em avaliações', () => {
    expect(caminhoInstrutor({ cursoRef: 'c1', secao: 'alunos', subAba: 'exercicios' }))
...
describe('janelas (modais) no endereço', () => {
  it('a janela entra e sai do endereço', () => {
    ...
    expect(caminhoInstrutor({ cursoRef: 'course-1', janela: 'nova-aula' }))
  ...
  it('a janela convive com seção e sub-aba', () => {
    expect(caminhoInstrutor({
      cursoRef: 'course-1', secao: 'avaliacoes', subAba: 'exercicios', janela: 'documentos',
  ...
  it('a janela sobrevive à ausência de curso', () => {
    ...
    expect(caminhoInstrutor({ cursoRef: null, janela: 'novo-curso' }))
  ...
  it('janela vazia não suja o endereço', () => {
    expect(caminhoInstrutor({ cursoRef: 'c1', janela: '' })).toBe('/inst/curso/c1');
    expect(caminhoInstrutor({ cursoRef: 'c1', janela: null })).toBe('/inst/curso/c1');
  });
});
...
describe('caminhos que não são do painel', () => {
  it('não confunde prefixo parecido', () => {
    ...
    expect(parseInstrutor('/institucional/curso/course-1').cursoRef).toBeNull();
    expect(parseInstrutor('/app').cursoRef).toBeNull();
    expect(parseInstrutor('/').cursoRef).toBeNull();
  });
});
