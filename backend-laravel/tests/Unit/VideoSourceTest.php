...
use App\Support\VideoProvider;
use App\Support\VideoSource;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

/**
 * Paridade manual com tests/frontend/videoSource.test.ts — ao alterar um caso
 * aqui, replicar lá (mesmo pacto de features.php ↔ features.ts).
 */
class VideoSourceTest extends TestCase
{
    private const CANONICAL = 'https://www.youtube.com/watch?v=dQw4w9WgXcQ';
    ...
    public function test_youtube_urls_canonicalizam_para_watch(string $url): void
    {
        $source = VideoSource::tryParse($url);

        $this->assertNotNull($source);
        $this->assertSame(VideoProvider::Youtube, $source->provider);
        $this->assertSame('dQw4w9WgXcQ', $source->videoId);
        $this->assertSame(self::CANONICAL, $source->canonicalUrl);
    }
    ...
    public function test_urls_de_arquivo_de_video_sao_aceitas(string $url): void
    {
        $source = VideoSource::tryParse($url);

        $this->assertNotNull($source);
        $this->assertSame(VideoProvider::File, $source->provider);
        $this->assertNull($source->videoId);
        $this->assertSame($url, $source->canonicalUrl);
    }
    ...
    public function test_urls_invalidas_sao_rejeitadas(?string $url): void
    {
        $this->assertNull(VideoSource::tryParse($url));
        $this->assertFalse(VideoSource::isValid($url));
    }
}
